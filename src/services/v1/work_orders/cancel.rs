...
use crate::{
    ...
    entities::{work_orders, work_order_state_history, cancel_reasons},
};
...
mod tests {
    ...
    fn test_decide_cancel_success() {
        ...
        let result = decide_cancel_work_order(work_order, closed_status_id, customer_id, 24, "Changed my mind".to_string(), None);
        assert!(result.is_ok());
    ...
    fn test_decide_cancel_wrong_customer() {
        ...
        let result = decide_cancel_work_order(work_order, 4, wrong_customer_id, 24, "Reason".to_string(), None);
