...
pub async fn update_closing_form_photo(
    ...
    let link = work_order_closing_image_links::Entity::find_by_id((image_id, id))
        .one(db.as_ref())
        .await?
...
#[utoipa::path(
    ...
    request_body(content_type = "multipart/form-data", description = "Signature image file"),
    responses(
...
pub async fn upload_closing_form_signature(
    ...
    // 1. Fetch data (I/O)
    let _work_order = work_orders::Entity::find_by_id(id)
        .one(db.as_ref())
        .await?
        .ok_or_else(|| AppError::NotFound("Work order not found".to_string()))?;
