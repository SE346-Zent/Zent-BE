...
pub(crate) async fn try_auto_assign_single(
    ...
    let active_status_id = match luts.account_statuses_by_name.get("Active") {
        Some(id) => *id,
        None => { tracing::warn!("Active account status not found in lookup tables"); return false; }
    };

    let technicians = match users::Entity::find().filter(users::Column::RoleId.eq(tech_role_id)).filter(users::Column::Province.eq(&province)).filter(users::Column::AccountStatus.eq(active_status_id)).all(db.as_ref()).await {
