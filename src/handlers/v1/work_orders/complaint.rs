...
use crate::model::requests::work_orders::complaint_request::ComplaintWorkOrderRequest;
use crate::model::responses::base::ApiResponse;

#[utoipa::path(
    post, path = "/api/v1/work_orders/{id}/complaint", request_body = ComplaintWorkOrderRequest,
    responses(
        (status = 200, description = "Complaint submitted successfully"),
...
pub async fn complaint(
    ...
    Path(id): Path<Uuid>,
    Json(payload): Json<ComplaintWorkOrderRequest>,
    ...
    let effect = crate::services::v1::work_orders::complaint::decide_complaint_work_order(
        work_order,
        auth.user.id,
        closed_status_id,
        payload.message,
    ...
    Ok(Json(ApiResponse::message_only(200, "Complaint submitted successfully")))
}
