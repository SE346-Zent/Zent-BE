...
pub fn build_cleanup_job(db: DatabaseConnection, lookup_tables: Arc<LookupTables>) -> Result<Job, anyhow::Error> {
    let job = Job::new_async("0 1/10 * * * *", move |_uuid, _l| {
        let db_clone = db.clone();
        let lut_clone = lookup_tables.clone();
        Box::pin(async move {
            info!("Running pending account cleanup job...");
            if let Err(e) = cleanup_pending_accounts(&db_clone, &lut_clone).await {
