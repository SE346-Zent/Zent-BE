...
use zent_be::entities::{part_catalog, part_conditions, parts, product_models, products};
...
fn integration_approval_state_machine_with_audit() {
    ...
    let r = deny_part::decide_deny_part(
        ...
        admin_id,
        "denied",
        "This part was already denied by the approver",
        now,
    );
    assert!(r.is_err());
}
